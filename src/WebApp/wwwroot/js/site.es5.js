...
function getInfoCardPositions(count, msgCount) {
    ...
    if (count <= 8) {
        ...
        return [Math.round(count / 2), count];
    } else if (count <= 32) {
        return [Math.round(count / 3), Math.round(count / 3 * 2), count];
    } else if (count <= 64) {
        return [Math.round(count / 4), Math.round(count / 4 * 2), Math.round(count / 4 * 3), count];
    } else if (count <= 96) {
        return [Math.round(count / 5), Math.round(count / 5 * 2), Math.round(count / 5 * 3), Math.round(count / 5 * 4), count];
    } else if (count <= 128) {
        return [Math.round(count / 6), Math.round(count / 6 * 2), Math.round(count / 6 * 3), Math.round(count / 6 * 4), Math.round(count / 6 * 5), count];
    } else {
        var positions = [];
...
function getInfoCard(msg) {
    ...
    var html = '<div class="col s12 m8 l3 prod">' + '<div class="card msg-card" style="background-color:' + tempColor + '; color: ' + tempForeColor + '">' + '<div class="card-content">' + '<span class="card-title">' + msg.Title + '</span>' + '<p>' + msg.Message + '</p>' + '</div>';

    if (msg.HasLinks) {
        html += '<div class="card-action">' + '<a href="' + msg.Link + '" target="_blank" style="color: ' + tempForeColor + '">' + msg.LinkText + '</a>' + '</div>';
    }
    ...
    $.ajax({
        ...
        success: function success(data) {
            ...
            modal.find('#main-img').fadeOut(function () {
                $(this).attr('src', data.images[0].sourceURLLarge).load(function () {
                    $(this).fadeIn();
                });
            });
            modal.find('.price-after').html('base price <b>$' + data.price.toFixed(2) + '</b><sup>*<sup>').css('opacity', '1').css('transition', '0.5s');
            modal.find('#modal-store-logo').attr('src', data.supplier.logo).load(function () {
                $(this).css('opacity', '1').css('transition', '0.5s');
            });
...
$(function () {
    ...
    $('#colorNumber').chromoselector({
        ...
        str2color: function str2color(str) {
            colorHiddenInput.val(str);
            return '#' + str;
        },
        color2str: function color2str(color) {
    ...
    var setPanelColors = function setPanelColors() {
    ...
    colorNameInput.typeahead(null, {
        ...
        source: dataSource,
        templates: {
            empty: ['<div class="empty-message">', 'unable to find any matching paints/colors', '</div>'].join('\n'),
